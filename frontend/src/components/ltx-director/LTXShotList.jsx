...
import { createSequence, getSequence, updateSequence, renderSequence, renderShot, stitchSequence, approveKeyframe, generateKeyframe } from "../../services/ltxSequenceService";
...
export default function LTXShotList({ globalConfig, onGlobalConfigChange }) {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        setSequence(local);
        if (local.project_id) {
          getSequence(local.project_id).then((remote) => {
            if (!dirtyRef.current) setSequence(remote);
          }).catch(() => { /* Keep the local draft when the backend is temporarily unavailable. */ });
        }
        return;
      } catch { /* migrate by starting fresh */ }
    }
  ...
  useEffect(() => {
    if (!sequence?.project_id) return undefined;
    const refresh = async () => {
      if (dirtyRef.current) return;
      try {
        const remote = await getSequence(sequence.project_id);
        if (!dirtyRef.current) setSequence(remote);
      } catch { /* A transient backend outage must not destroy the local editor state. */ }
    };
    const timer = window.setInterval(refresh, 3000);
    return () => window.clearInterval(timer);
  }, [sequence?.project_id]);
  const markDirty = () => { dirtyRef.current = true; };
  const patchShot = (id, patch) => { markDirty(); setSequence((current) => ({ ...current, shots: current.shots.map((shot) => shot.id === id ? { ...shot, ...patch } : shot) })); };
  const move = (index, delta) => { markDirty(); setSequence((current) => { const shots = [...current.shots]; const target = index + delta; if (target < 0 || target >= shots.length) return current; [shots[index], shots[target]] = [shots[target], shots[index]]; return { ...current, shots: shots.map((shot, order) => ({ ...shot, order })) }; }); };
  const add = () => { markDirty(); setSequence((current) => ({ ...current, shots: [...current.shots, newShot(current.shots.length, { ...current.global, prompt: "" })] })); };
  const duplicate = (shot) => { markDirty(); setSequence((current) => ({ ...current, shots: [...current.shots, { ...shot, id: crypto.randomUUID(), order: current.shots.length, name: `${shot.name} copy`, status: "pending", output_path: null, final_frame_path: null, attempt_number: 0 }] })); };
  const save = async (value = sequence, announce = true, manageBusy = true) => { if (manageBusy) setBusy(true); setError(""); try { const saved = value.project_id ? await updateSequence(value.project_id, value) : await createSequence(value); dirtyRef.current = false; setSequence(saved); if (announce) setMessage("Sequence saved"); return saved; } catch (err) { if (err.status === 409 && value.project_id) { try { const remote = await getSequence(value.project_id); dirtyRef.current = false; setSequence(remote); } catch { /* Keep the draft visible if refresh also fails. */ } setError("Sequence changed elsewhere; the latest backend version was loaded. Review your edits before saving again."); } else setError(err.message); return null; } finally { if (manageBusy) setBusy(false); } };
  const refresh = async () => { if (!sequence?.project_id) return; setBusy(true); setError(""); try { const remote = await getSequence(sequence.project_id); dirtyRef.current = false; setSequence(remote); setMessage("Loaded latest backend sequence"); } catch (err) { setError(err.message); } finally { setBusy(false); } };
  const run = async (action) => { setBusy(true); setError(""); try { const current = await save(sequence, false, false); if (!current?.project_id) return; const updated = await action(current.project_id); dirtyRef.current = false; if (updated?.shots) setSequence(updated); setMessage("Request accepted"); } catch (err) { setError(err.message); } finally { setBusy(false); } };
  const approve = async (shot) => {
    if (!sequence.project_id || !shot.keyframe_asset_path) return;
    setBusy(true); setError("");
    try { const updated = await approveKeyframe(sequence.project_id, shot.id, shot.keyframe_asset_path); dirtyRef.current = false; setSequence(updated); setMessage("Keyframe approved and locked"); }
    catch (err) { setError(err.message); } finally { setBusy(false); }
  };
  const generate = async (shot, regenerate = false) => {
    if (!sequence.project_id) return;
    setBusy(true); setError("");
    try { const updated = await generateKeyframe(sequence.project_id, shot.id, regenerate); dirtyRef.current = false; setSequence(updated); setMessage(regenerate ? "Keyframe regenerated; approve it before animation" : "Keyframe generated; approve it before animation"); }
    catch (err) { setError(err.message); } finally { setBusy(false); }
  };
  ...
  return <Card variant="outlined"><CardContent><Stack spacing={1.5}>
      ...
      <Stack direction="row" spacing={1} flexWrap="wrap"><Button size="small" variant="outlined" onClick={add} startIcon={<AddIcon />}>Add shot</Button><Button size="small" variant="outlined" disabled={busy} onClick={refresh}>Refresh latest</Button><Button size="small" variant="outlined" disabled={busy} onClick={() => save()}>Save</Button><Button size="small" variant="contained" disabled={busy} onClick={() => run(renderSequence)} startIcon={<PlayArrowIcon />}>Render pending</Button><Button size="small" variant="outlined" disabled={busy} onClick={() => run(stitchSequence)}>Stitch hard cuts</Button></Stack>
    </Stack>
